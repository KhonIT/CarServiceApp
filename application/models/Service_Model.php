...
class Service_Model extends CI_Model{
	 ...
	 public function Get_By_ID($id){
		  $sql = 'select service_id,service_name,price  from service where is_show = 1 and service_id = ?';
		  $query = $this->db->query($sql, array($id));
		  log_message('debug', sprintf('Found %b row with service ID %s', $query->num_rows(), $id));
		  return $query->row_array();// return one row
	 } 
	 public function Get_All(){
		  $sql = 'select service_id,service_name,price  from service where is_show = 1';
